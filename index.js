...
app.post('/meta-webhook', async (req, res) => {
    ...
    try {
        // 🔍 DEBUG LOG: See exactly what Meta is sending
        console.log('[Meta RAW Payload]:', JSON.stringify(req.body, null, 2));
        ...
        if (!msgObj) {
            console.log('[Meta] No message object found in this payload.');
            return;
        }

        if (msgObj.type !== 'text') {
            console.log(`[Meta] Received non-text message type: ${msgObj.type}`);
            return;
        }
        ...
        console.log(`[Meta] Processed Message: from=${from} msg=${msg}`);

        const reply = await handleBotMessage(from, msg);
        if (reply) {
            console.log(`[Meta] Attempting to send reply to ${from}...`);
            const success = await sendMetaMessage(from, reply);
            console.log(`[Meta] Send Status: ${success ? '✅ SUCCESS' : '❌ FAILED'}`);
        } else {
            console.log('[Meta] No reply generated by bot logic.');
        }
    } catch (err) {
